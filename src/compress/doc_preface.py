...
o arquivo nem mandam 3000 linhas ao modelo.
  ...
  python -m src.compress.doc_preface docs/propostas-melhoria-limites-atuais.md
  python -m src.compress.doc_preface FILE.md --apply
  python -m src.compress.doc_preface FILE.md --lookup "história gaps"
  python -m src.compress.doc_preface FILE.md --show 22-code-evidence-spec
"""
...
from src.repos.servicos import fold
...
def render_preface(index: DocIndex, *, sidecar_name: str) -> str:
    lines: list[str] = [
        ...
        "",
        "Gerado por `python -m src.compress.doc_preface`. Este bloco é a **tabela de roteamento**.",
...
def sidecar_payload(index: DocIndex) -> dict[str, Any]:
    return {
        "document": index.path,
        "generated_by": "src.compress.doc_preface",
